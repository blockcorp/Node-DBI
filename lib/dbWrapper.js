...
{
  return this._insert(tableName, data, false, callback);
};


/**
 * Inserts a table row with specified data, using REPLACE INTO
 *
 * @param {tableName}  tableName          The table to insert data into.
 * @param {Object}  data                  Column-value pairs.
 * @param {Function|null}  callback       The callback function receives the error, if any, and the number of affected rows
 * @api public
 */
DBWrapper.prototype.replace = function( tableName, data, callback )
{
  return this._insert(tableName, data, true, callback);
};
 ...
 * @param {tableName}  tableName          The table to insert data into.
 * @param {Object}  data                  Column-value pairs.
 * @param {Boolean}  replaceInto          Use REPLACE INTO instead of INSERT INTO
 * @param {Function|null}  callback       The callback function receives the error, if any, and the number of affected rows
 * @api private
 */
DBWrapper.prototype._insert = function( tableName, data, replaceInto, callback )
{
  ...
  // SQL initialization
  var sql = replaceInto ? 'REPLACE INTO ' : 'INSERT INTO ';
  sql += this._adapter.escapeTable(tableName);
